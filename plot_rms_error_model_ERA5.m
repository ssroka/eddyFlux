...
for j = 2
    ...
    for i = 1:length(abCD_fac_vec)
        ...
        for y = 1:length(year_vec)
            year = year_vec(y);
            load(sprintf('opt_abCD_%sfilt_%s_L_%d_box%d_%s_%d',con_str,filter_type,L/1000,box_num,model_str,year),'abCD');
            load(sprintf('mean_rms_err_%s_%d%s',model_str,year,abCD_fac_str),'mean_rms')
            rms_er(i,y) = mean_rms;
            abCD_mat(:,y) = abCD(:);
        end
    ...
    pause(1)% don't remove not saving properly without these
    print(sprintf('%simgs/rms_error_%d_%s_box%d_%s_%d_abCDFAC_%s',data_base,L/1000,filter_type,box_num,model_str,year,abCD_fac_str),'-dpdf')
    ...
    update_figure_paper_size()
    pause(1)% don't remove not saving properly without these
    print(sprintf('%simgs/rms_error_%d_%s_box%d_%s_beta_%d_abCDFAC_%s',data_base,L/1000,filter_type,box_num,model_str,year,abCD_fac_str),'-dpdf')
    ...
    update_figure_paper_size()
    print(sprintf('%simgs/rms_error_erbar_%d_%s_box%d_%s_%d_abCDFAC_%s',data_base,L/1000,filter_type,box_num,model_str,year,abCD_fac_str),'-dpdf')
