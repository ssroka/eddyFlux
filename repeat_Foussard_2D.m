...
[gradSSTX,gradSSTY] = gradient(SST_anomaly,x(2)-x(1));

% compute the curl
subplot(2,2,2)
[cz,cav] = curl(X,Y,tau,0*tau);
imagesc(c); colorbar
title('\nabla x \tau')
set(gca,'fontsize',18)
...
%% compute flux ratio
u_mean = repmat(mean(u,2),1,N);
qo_mean = repmat(mean(qo,2),1,N);
qa_mean = repmat(mean(qa,2),1,N);
u_prime = u-u_mean;
qo_prime = qo-qo_mean;
qa_prime = qa-qa_mean;

Numerator = u_prime.*(qo_prime-qa_prime);
Denominator = u_mean.*(qo_mean-qa_mean);
figure(4)
plot(x,(Numerator./Denominator)')
