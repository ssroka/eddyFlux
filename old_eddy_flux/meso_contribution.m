...
alpha_vec =  [1.27045 1.5335 1.94 2.111 2.6630];
...
% ---------- Parameters --------------
% filter parameters
L_vec = [500 750 1000]*1000; % [m] filtering box edge length
...
for ii_year = 1:length(year_vec)
    ...
    L = L_vec(ii_L); % [m] filtering box edge length
    ...
    for tt = 1:length(time)
        ...
        dq_no_eddy = smooth2a(qo_patch - qa_patch,Ny,Nx);
        
        DT_eddy = SST_patch(:,:,tt)-t2m_patch(:,:,tt);
        DT_no_eddy = smooth2a(SST_patch(:,:,tt)-t2m_patch(:,:,tt),Ny,Nx);
        
        sshf_eddy(:,:,tt) = rho_a.*c_p_air.*CD_ref.*(1+as_est(SST_prime(:,:,tt)).*SST_prime(:,:,tt)).*U_mag(:,:,tt).*(DT_eddy);
        slhf_eddy(:,:,tt) = rho_a.*Lv.*CD_ref.*(1+aL_est(SST_prime(:,:,tt)).*SST_prime(:,:,tt)).*U_mag(:,:,tt).*(dq_eddy);
        
        sshf_sm(:,:,tt) = rho_a.*c_p_air.*CD_ref.*U_mag(:,:,tt).*(DT_no_eddy);
        slhf_sm(:,:,tt) = rho_a.*Lv.*CD_ref.*U_mag(:,:,tt).*(dq_no_eddy);
...
figure(1)
  plot_eddy_flux_diff   
  fig_str = sprintf('eddy_m_no_eddy_%d_L_%d_km',time(end).Year,L/1000);
  base_loc = '/Users/ssroka/MIT/Research/eddyFlux/imgs_channel/';
  file_name = [base_loc fig_str];
  print(file_name,'-dpng')
  savefig(file_name)
  clf

g = var(SST_prime,0,3);
M(ii_year,:) = [time(end).Year nanmean(sshf_diff(:)+slhf_diff(:)) nanmedian(sshf_diff(:)+slhf_diff(:)) nanmedian(g(:))];
figure(2)
plot_var_vs_diff_flux
end
  fig_str = sprintf('eddy_m_no_eddy_%d_L_%d_km_VAR',time(end).Year,L/1000);
  base_loc = '/Users/ssroka/MIT/Research/eddyFlux/imgs_channel/';
  file_name = [base_loc fig_str];
  print(file_name,'-dpng')
  savefig(file_name)
