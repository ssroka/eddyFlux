...
%         U = u_bar/(L_km/2)*(Y-L_km/2)+uT_coeff*T_prime;
    ...
    case 'baroclinic_wave'
        ...
        U = U/2; % to better match real data
        V = V/3; % to better match real data
end
if plot_flag
    figure(20)
...
%                 hold on
                ...
                figure(9)
                subplot(3,2,1)
                contourf(x,x,Q_L(:,:,2),20)
                colorbar
                xlabel(' km ')
                ylabel(' km ')
                t_str = sprintf('$$Q_L^{eddy} \\alpha = $$ %2.3f',i_param);
                title(t_str,'interpreter','latex')
                set(gca,'ydir','normal','fontsize',20)
                set(gcf,'color','w','position',[ 84          50        1156         7329])
            
                subplot(3,2,2)
                contourf(x,x,Q_s(:,:,2),20)
                colorbar
                xlabel(' km ')
                ylabel(' km ')
                t_str = sprintf('$$Q_s^{eddy} \\alpha = $$ %2.3f',i_param);
                title(t_str,'interpreter','latex')
                set(gca,'ydir','normal','fontsize',20)
                set(gcf,'color','w','position',[ 84          50        1156         7329])
                ...
                subplot(3,2,3)
                contourf(x,x,Q_L(:,:,1),10)
                colorbar
                xlabel(' km ')
                ylabel(' km ')
                t_str = sprintf('$$Q_L^{ctrl} \\alpha = $$ %2.3f',i_param);
                title(t_str,'interpreter','latex')
                set(gca,'ydir','normal','fontsize',20)
                set(gcf,'color','w','position',[ 84          50        1156         7329])
            
                subplot(3,2,4)
                contourf(x,x,Q_s(:,:,1),20)
                colorbar
                xlabel(' km ')
                ylabel(' km ')
                t_str = sprintf('$$Q_s^{ctrl} \\alpha = $$ %2.3f',i_param);
                title(t_str,'interpreter','latex')
                set(gca,'ydir','normal','fontsize',20)
                set(gcf,'color','w','position',[ 84          50        1156         7329])
  
                                subplot(3,2,5)
                contourf(x,x,Q_L(:,:,1)-Q_L(:,:,2),20)
                colorbar
                xlabel(' km ')
                ylabel(' km ')
                t_str = sprintf('$$Q_L^{ctrl}-Q_L^{eddy} \\alpha = $$ %2.3f',i_param);
                title(t_str,'interpreter','latex')
                set(gca,'ydir','normal','fontsize',20)
                set(gcf,'color','w','position',[ 84          50        1156         7329])
            
                subplot(3,2,6)
                contourf(x,x,Q_s(:,:,1)-Q_s(:,:,2),10,'linewidth',2)
                colorbar
                xlabel(' km ')
                ylabel(' km ')
                t_str = sprintf('$$Q_s^{ctrl}-Q_s^{eddy} \\alpha = $$ %2.3f',i_param);
                title(t_str,'interpreter','latex')
                set(gca,'ydir','normal','fontsize',20)
                set(gcf,'color','w','position',[ 84          50        1156         7329])
...
print('/Users/ssroka/MIT/Research/eddyFlux/imgs_channel/channel_eddy_minus_ctrl','-dpng')
print('/Users/ssroka/MIT/Research/eddyFlux/imgs_channel/channel_ctrl_Q_T','-dpng')
print('/Users/ssroka/MIT/Research/eddyFlux/imgs_channel/SST_prime_box','-dpng')
