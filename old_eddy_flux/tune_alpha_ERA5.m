...
for i_param = 1:N_param
    ...
    switch i_param
        case 1
            ...
            test_param = [0.001 0.01 0.05 0.1 0.5];
    ...
    for ind_avgHeat = 1:20
        for i = 1:length(test_param)
            ...
            SST_ind = true(size(X));%(X>3500)&(X<5500)&(Y<5500)&(Y>3500);
            ...
            if plot_flag
                figure(5)
                subplot(1,2,2)
                plot(aw_2_plt(:)*(1000)*(100),abs(d_2_plt(:))*1e7,'o')
                hold on
            ...
            zonal_SHF_diff(:,i,ind_avgHeat) = mean((Q_s(:,:,2)-Q_s(:,:,1))./Q_s(:,:,1),2);
            zonal_LHF_diff(:,i,ind_avgHeat) = mean((Q_L(:,:,2)-Q_L(:,:,1))./Q_L(:,:,1),2);
            ...
            if (plot_flag || plot_Heat_Flux) && (ind_avgHeat==1)
                ...
                if plot_rel_diff
                    plot(mean((Q_s(:,:,2)-Q_s(:,:,1))./Q_s(:,:,1),2),x,'linewidth',2,'color',cmap_mat(i,:),...
                ...
                xlabel('W m^{-2}')
                ylabel('km')
                str1 = 'Zonal Mean $(Q_s^{eddy} - Q_s^{ctrl})/Q_s^{ctrl}$';
                ...
                if plot_rel_diff
                    plot(mean((Q_L(:,:,2)-Q_L(:,:,1))./Q_L(:,:,1),2),x,'linewidth',2,'color',cmap_mat(i,:),...
                        'displayname',sprintf('%s = %2.3f',test_param_str,test_param(i)))
                else
                ...
                xlabel('W m^{-2}')
                ylabel('km')
                str1 = 'Zonal Mean $(Q_L^{eddy} - Q_L^{ctrl})/Q_L^{ctrl}$';
                title(str1,'interpreter','latex')
                lh = legend('-dynamiclegend');
                set(lh,'location','eastoutside')
                set(gca,'fontsize',18)
