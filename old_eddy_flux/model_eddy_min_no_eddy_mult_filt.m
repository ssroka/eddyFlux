...
%     tot_flux_1 = sshf_as_constant + slhf_aL_constant;
    ...
    T_o_prime = zeros(sum(patch_lon),sum(patch_lat),length(time));
...
[M] = my_smoother_mat(sum(patch_lon),sum(patch_lat),Ny,Nx,NaN_inds);
filter_fxn =@(x) smooth_mat(x,M);
    ...
    for tt = 1:length(time)
        ...
        dq_no_eddy = filter_fxn(qo_patch - qa_patch);
        Umag_no_eddy = filter_fxn(U_mag(:,:,tt));
        
        % add the smooth U_mag 
        
        DT_eddy = SST_patch(:,:,tt)-t2m_patch(:,:,tt);
        DT_no_eddy = filter_fxn(SST_patch(:,:,tt)-t2m_patch(:,:,tt));
        T_o_prime(:,:,tt) = SST_patch(:,:,tt) - filter_fxn(SST_patch(:,:,tt));
        T_o_prime_sq_sm = filter_fxn(T_o_prime.^2);
        
        T_a_prime = t2m_patch(:,:,tt) - filter_fxn(t2m_patch(:,:,tt));
        T_a_prime_T_o_prime_sm = filter_fxn(T_a_prime.*T_o_prime);
        
        sshf_eddy(:,:,tt) = rho_a.*c_p_air.*CD_ref_s.*(1+as_est(T_o_prime(:,:,tt)).*T_o_prime(:,:,tt)).*U_mag(:,:,tt).*(DT_eddy);
        slhf_eddy(:,:,tt) = rho_a.*Lv.*CD_ref_L.*(1+aL_est(T_o_prime(:,:,tt)).*T_o_prime(:,:,tt)).*U_mag(:,:,tt).*(dq_eddy);
        
        sshf_sm(:,:,tt) = rho_a.*c_p_air.*CD_ref_s.*Umag_no_eddy.*(DT_no_eddy);
        slhf_sm(:,:,tt) = rho_a.*Lv.*CD_ref_L.*Umag_no_eddy.*(dq_no_eddy);
        
        sshf_diff(:,:,tt) = filter_fxn(sshf_eddy(:,:,tt) - sshf_sm(:,:,tt));
        slhf_diff(:,:,tt) = filter_fxn(slhf_eddy(:,:,tt) - slhf_sm(:,:,tt));
        
        term1(:,:,tt) = rho_a.*c_p_air.*CD_ref_s.*Umag_no_eddy.*DT_no_eddy;
        term2(:,:,tt) = rho_a.*c_p_air.*CD_ref_s.*as_est(T_o_prime(:,:,tt)).*Umag_no_eddy.*T_o_prime_sq_sm;
        term3(:,:,tt) = -rho_a.*c_p_air.*CD_ref_s.*as_est(T_o_prime(:,:,tt)).*Umag_no_eddy.*T_a_prime_T_o_prime_sm;
        
    end
    close all
    plot_model_eddy_min_no_eddy
    plot_terms_fm_deriv
    save(sprintf('%d_model_eddy_min_no_eddy_results_my_filt.mat',year))
end
