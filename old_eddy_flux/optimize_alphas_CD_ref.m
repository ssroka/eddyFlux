function [mean_er_1_sm] = optimize_alphas_CD_ref(alpha_CD,year)
...
error_metric_flag  = 'point'; % 'box' or 'point'
err_box_lat = [32 38];
err_box_lon = [130 160];
...
% qo_patch = zeros(size(SST_patch,1),size(SST_patch,2),size(SST_patch,3));
% qa_patch = zeros(size(SST_patch,1),size(SST_patch,2),size(SST_patch,3));
...
for tt = 1:length(time)
    ...
    % ---- qo ----
    qo_patch(:,:,tt) = SAM_qsatWater(SST_patch(:,:,tt), P0_patch(:,:,tt)) ;
...
%     slhf_aL_constant(:,:,tt) = rho_a.*Lv.*CD_ref.*(1+aL_est(SST_prime(:,:,tt)).*SST_prime(:,:,tt)).*U_mag(:,:,tt).*((qo_patch(:,:,tt)-qa_patch(:,:,tt)));
...
% if as_est(0) == aL_est(1) % if the alpha's are the same constant
    ...
    if strcmp(error_metric_flag,'point')
        ...
        er_1_sm = (tot_flux_1(err_box_bnds_lon,err_box_bnds_lat,:)-sshf_patch(err_box_bnds_lon,err_box_bnds_lat,:)).^2;
        
        mean_er_1_sm = sqrt(nanmean(nanmean(nanmean(er_1_sm))));
