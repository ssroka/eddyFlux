function [X, XHP, f, y, y2] = fftf(t, x, varargin)
% fftf - fft filter;
...
[y2,yHP]=filterfft(f, y, cutoff_freq, my_freqs); % filter amplitudes
...
X=X(1:N)/max(X);
XHP=ifft(yHP); % inverse transform
XHP=XHP(1:N)/max(XHP);
...
function [y2,yHP]=filterfft(f, y, cutoff, wins)
nf=length(f);
ny=length(y);
if ~(ny/2+1 == nf),
...
if ~isempty(cutoff)
    ...
    ind2=find(f>cutoff);
    yHP(ind2) = y(ind2); % insert required elements
else
    y2=y;
end
...
for k=nf+1:ny,
    ...
    yHP(k) = conj(yHP(mod(ny-k+1,ny)+1)); % formula from the help of ifft
end
return