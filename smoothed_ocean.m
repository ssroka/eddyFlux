...
for year = 1
    ...
    Q_s_so  = zeros(sum(patch_lon),sum(patch_lat),length(time));
    Q_L_so  = zeros(sum(patch_lon),sum(patch_lat),length(time));
...
%       SST_prime(:,:,tt) = SST_patch(:,:,tt) - SST_patch_CTRL;
        ...
        % ---- CD ----
        Q_s_so(:,:,tt) = rho_a.*c_p_air.*CDs.*U_mag(:,:,tt).*(DT_patch(:,:,tt));
        Q_L_so(:,:,tt) = rho_a.*Lv.*CDL.*U_mag(:,:,tt).*(qo_patch-qa_patch);
...
%{
    ...
    figure
    contourf(lon(patch_lon),lat(patch_lat),nanmean(Q_s_so,3)')
    colorbar
    xlabel(' Deg lon ')
    ylabel(' Deg lat ')
    title(sprintf('Smoothed Ocean $$Q_s$$ [W/$$m^{-2}$$] \n(mean DJFM %d) calculated with $$C_D^s$$',time(end).Year),'interpreter','latex')
    set(gca,'ydir','normal','fontsize',20)
    set(gcf,'color','w')

    figure
    contourf(lon(patch_lon),lat(patch_lat),nanmean(Q_L_so,3)',30,'k')
    colorbar
    xlabel(' Deg lon ')
    ylabel(' Deg lat ')
    p_str = '%';
    t_str = sprintf('Smoothed Ocean $$Q_L$$ [W/$$m^{-2}$$] \n(mean DJFM %d) calculated with $$C_D^L$$',time(end).Year);
...
%{
print('/Users/ssroka/MIT/Research/eddyFlux/imgs_channel/era5_Qs_so_03','-dpng')
print('/Users/ssroka/MIT/Research/eddyFlux/imgs_channel/era5_QL_so_03','-dpng')

print('/Users/ssroka/MIT/Research/eddyFlux/imgs_channel/era5_Qs_so_07','-dpng')
print('/Users/ssroka/MIT/Research/eddyFlux/imgs_channel/era5_QL_so_07','-dpng')
